...
public class ValidatorB extends Validator{
    ...
    public BookingDto handle(ReservationRequest reservationRequest) {
        ...
        String check = null;
        for (ReservationDto reservationDto : reservationRequest.getReservationDtos()){
            check = check(reservationDto);
            if (check != null) break;
        }

        if (check != null) return errorResponse(check);

        return checkNext(reservationRequest);
    }

    private String check(ReservationDto reservationDto){
        LocalDateTime start =
